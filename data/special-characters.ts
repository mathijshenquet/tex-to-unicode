const simpleModifier = function (modifier: string) {
    return (char: string) => {
        return char + modifier;
    }
};

function modifyStringFunction(conversionTable: {[k: string]: string}, _default_append?: string) {
    return (string: string) => {
        ...
        if (!_default_append) throw new Error("I do not know how to modify the following string: " + string + ". "
            + "Change your TeX file or submit a feature request at https://github.com/digitalheir/latex-to-unicode.js/issues.");
...
function tieLetters(chars: string) {
    //o͡o
    return chars.charAt(0) + '͡' + chars.substring(1);
}
function circledA(char: string) {
    if (char == 'a') {
        return 'å';
